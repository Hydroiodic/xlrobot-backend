...
typedef struct {
    ...
    JointMonitorData
        jointMonitorData[JAKA_ROBOT_MAX_JOINT]; ///< 机器人7个关节的监测数据
} RobotMonitorData;
...
typedef struct {
    ...
    double actTorque
        [6]; ///< 力矩传感器实际接触力值（勾选初始化时）或原始读数值（勾选不初始化时）
...
typedef struct {
    ...
    TorqSensorMonitorData
        torq_sensor_monitor_data; ///< 机器人力矩传感器状态监测数据
 ...
 */
typedef struct {
    int opt; ///< 柔顺方向，可选值为 1 2 3 4 5 6分别对应 fx fy fz mx my
             ///< mz,0代表没有勾选
    double
        ft_user; ///< 用户用多大的力才能让机器人的沿着某个方向以最大速度进行运动
...
typedef struct {
    ...
    int slaveId; // 当通道模式设置为Modbus
                 // RTU时，需额外指定Modbus从站节点ID，其余模式可忽略
