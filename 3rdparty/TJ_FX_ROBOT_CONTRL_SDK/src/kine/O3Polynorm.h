...
/*/
        3-Order Polynome	:	Y = Pn(X) = A * X^3 + B * X^2 + + C * X
+ D Parameters of Pn(x) :	double PnPara[4] = {A,B,C,D} FD
:	First derivative SD					:	Second
Derivative SOC					:	Second-order Continuity
...
class CO3Polynorm {
  public:
    ...
    /*/ Cal Parameters of Pn(x) By two Points
     * :p0{x[0],Pn(x[0]),FD(x[0]),SD(x[0])} p1{x[1],Pn(x[1])}/*/
    static bool CalPnParaSoC(double p0_XYO1O2[4], double p1_XY[2],
                             double ret_PnPara[4]);
};
