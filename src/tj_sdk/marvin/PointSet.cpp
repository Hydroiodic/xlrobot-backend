...
bool CPointSet::OnSubNoEmpty(long serial, double target_v, double eps,
                             CPointSet *ret) {
...
bool CPointSet::OnSubZone(long serial, double min_v, double max_v,
                          CPointSet *ret) {
    if (this->m_PointType == PotT_BEGIN) {
        return false;
    }
...
bool CPointSet::OnGetSubAVG(long serial, long start, long end,
                            double &ret_avg) {
    double dnum = 0;
    double tv = 0;
...
bool CPointSet::OnFindByTagGetAvg(long serial, double tag, long startpos,
                                  long &ret_start, long &ret_end, long a_serial,
                                  double &ret_avg) {
...
bool CPointSet::OnFindByTag(long serial, double tag, long startpos,
                            long &ret_start, long &ret_end) {
    long num = OnGetPointNum();
    if (startpos < 0) {
        startpos = 0;
    }
...
bool CPointSet::OnSub(long head_size, long head_repeatnum, long tail_size,
                      long tail_repeatnum) {
    long num = this->OnGetPointNum();
...
bool CPointSet::OnSubAtValue(long serial, double target_v, double eps,
                             CPointSet *ret) {
    if (this->m_PointType == PotT_BEGIN) {
        return false;
    }
...
PoinType CPointSet::OnGetType() { return m_PointType; }
...
long CPointSet::OnGetPointNum() { return m_Points.OnGetNum(); }
...
bool CPointSet::OnSave(FILE *fp) {
    ...
    if (strlen(m_Tag) == 0) {
        ...
        if (strlen(m_SubTag) == 0) {
            fprintf(fp, "PoinType=%d@%dT%d%s%c", m_PointType, num,
                    strlen(m_Tag), m_Tag, r);
        } else {
            fprintf(fp, "PoinType=%d@%dT%d%sST%d%s%c", m_PointType, num,
                    strlen(m_Tag), m_Tag, strlen(m_SubTag), m_SubTag, r);
        }
    }
...
bool CPointSet::OnAdd(long src_col_serial1, long src_col_serial2,
                      long tgt_col_serial) {
...
bool CPointSet::OnCalDif(long src_col_serial, long tgt_col_serial,
                         long skip_num) {
...
void PSlinearSmooth3(double in[], double out[], int size) {
    ...
    if (size < 3) {
        ...
        out[size - 1] =
            (5.0 * in[size - 1] + 2.0 * in[size - 2] - in[size - 3]) / 6.0;
    }
}
...
void PSlinearSmooth7(double in[], double out[], int size) {
    ...
    if (size < 7) {
        ...
        out[1] = (5.0 * in[0] + 4.0 * in[1] + 3 * in[2] + 2 * in[3] + in[4] -
                  in[5]) /
                 14.0;
        ...
        out[size - 3] =
            (7.0 * in[size - 1] + 6.0 * in[size - 2] + 5.0 * in[size - 3] +
             4.0 * in[size - 4] + 3.0 * in[size - 5] + 2.0 * in[size - 6] +
             in[size - 7]) /
            28.0;

        out[size - 2] =
            (5.0 * in[size - 1] + 4.0 * in[size - 2] + 3.0 * in[size - 3] +
             2.0 * in[size - 4] + in[size - 5] - in[size - 6]) /
            14.0;

        out[size - 1] =
            (13.0 * in[size - 1] + 10.0 * in[size - 2] + 7.0 * in[size - 3] +
             4.0 * in[size - 4] + in[size - 5] - 2.0 * in[size - 6] -
             5.0 * in[size - 7]) /
            28.0;
    }
}
