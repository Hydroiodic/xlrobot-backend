...
bool CTCPAgent::OnCheckLink() { return m_bLinkTag; }
...
void CTCPAgent::NetLoop(void *p) {
    ...
    while (1) {
        ...
        long rcvn =
            recv(sunit->m_iSocket, &sunit->SrvUtText[0], MAX_TCP_BUFSIZE, 0);
...
bool CTCPAgent::OnLinkTo(unsigned char ip1, unsigned char ip2,
    ...
    long tret = setsockopt(m_iSocket, SOL_SOCKET, SO_BINDTODEVICE,(char
*)&interface, sizeof(interface)); if ( tret != 0 )
{
...
/*/
    // 向服务器发出连接请求
    if (connect(m_iSocket, (struct sockaddr *)&addrSrv, sizeof(addrSrv)) ==
        INVALID_SOCKET) {
    ...
    DWORD id;
    HANDLE reth = CreateThread(
        NULL, 0, (unsigned long(__stdcall *)(void *))NetLoop, this, 0, &id);
    ...
    m_LoopThreadHandle = pthread_create(&pthread_t_Listen, &attrtNet,
                                        (void *(*)(void *))NetLoop, this);
...
bool CTCPAgent::OnSend(char *buf, long size) {
    if (m_bLinkTag == false || buf == NULL || size <= 0 ||
        m_iSocket == INVALID_SOCKET) {
        return false;
    }
    long slen;
