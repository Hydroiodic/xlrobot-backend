...
FX_BOOL LOADMvCfg(FX_CHAR *path, FX_INT32L TYPE[2], FX_DOUBLE GRV[2][3],
                  FX_DOUBLE DH[2][8][4], FX_DOUBLE PNVA[2][7][4],
                  FX_DOUBLE BD[2][4][3], FX_DOUBLE Mass[2][7],
                  FX_DOUBLE MCP[2][7][3], FX_DOUBLE I[2][7][6]) {
    ...
    for (i = 0; i < 2; i++) {
        if (fscanf(fp, "%ld,%lf,%lf,%lf,%c", &TYPE[i], &GRV[i][0], &GRV[i][1],
                   &GRV[i][2], &c) != 5) {
            fclose(fp);
            return FX_FALSE;
        }
        ...
        for (j = 0; j < 7; j++) {
            if (fscanf(fp,
                       ...
                       &PNVA[i][j][1], &PNVA[i][j][0], &PNVA[i][j][2],
                       &PNVA[i][j][3], &Mass[i][j], &MCP[i][j][0],
                       &MCP[i][j][1], &MCP[i][j][2], &I[i][j][0], &I[i][j][1],
                       &I[i][j][2], &I[i][j][3], &I[i][j][4], &I[i][j][5],
                       &c) != 19) {
                fclose(fp);
                return FX_FALSE;
            }
        ...
        if (fscanf(fp, "%lf,%lf,%lf,%lf,%c", &DH[i][7][0], &DH[i][7][1],
                   &DH[i][7][2], &DH[i][7][3], &c) != 5) {
            fclose(fp);
            return FX_FALSE;
        }
        if (c != 0x0a) {
        ...
        for (j = 0; j < 4; j++) {
            if (fscanf(fp, "%lf,%lf,%lf,%c", &BD[i][j][0], &BD[i][j][1],
                       &BD[i][j][2], &c) != 4) {
                fclose(fp);
                return FX_FALSE;
            }
...
FX_BOOL FX_Robot_Init_Type(FX_INT32L RobotSerial, FX_INT32L RobotType) {
    ...
    if (RobotType == FX_ROBOT_TYPE_DL) {
        ...
        m_Robot[RobotSerial].m_KineSPC =
            (FX_VOID *)&(m_Robot_SPC_DL[RobotSerial]);
        return FX_TRUE;
        ...
        m_Robot[RobotSerial].m_KineSPC =
            (FX_VOID *)&(m_Robot_SPC_Pilot[RobotSerial]);
...
FX_BOOL FX_Init_Robot_Kine_Pilot_SRS(FX_INT32L RobotSerial,
                                     FX_DOUBLE DH[8][4]) {
...

    {
        ...
        FX_PGMatrixInv(pRobot->m_KineBase.m_Flange,
                       pRobot->m_KineBase.m_InvFlange);
    }
...
FX_BOOL FX_Init_Robot_Kine_Pilot_CCS(FX_INT32L RobotSerial,
                                     FX_DOUBLE DH[8][4]) {
...
FX_BOOL FX_Init_Robot_Lmt_CCS(FX_INT32L RobotSerial, FX_DOUBLE PNVA[7][4],
                              FX_DOUBLE J67[4][3]) {
...
FX_BOOL FX_Robot_Init_Lmt(FX_INT32L RobotSerial, FX_DOUBLE PNVA[7][4],
                          FX_DOUBLE J67[4][3]) {
...
FX_VOID FX_XYZMRot(FX_DOUBLE L[4][4], FX_DOUBLE cosv, FX_DOUBLE sinv,
                   FX_DOUBLE T[4][4]) {
    FX_INT32L i;
    for (i = 0; i < 3; i++) {
...
FX_BOOL FX_Robot_Kine_Piolt(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                            FX_DOUBLE pgos[4][4]) {
    ...
    for (i = 0; i < 7; i++) {
        ...
        FX_XYZMRot(pRobot->m_KineBase.m_AxisRotBase[i], cosv, sinv,
                   pRobot->m_KineBase.m_AxisRotTip[i]);
    }

    FX_M44Copy(pRobot->m_KineBase.m_AxisRotTip[0],
               pRobot->m_KineBase.m_JointPG[0]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[0],
              pRobot->m_KineBase.m_AxisRotTip[1],
              pRobot->m_KineBase.m_JointPG[1]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[1],
              pRobot->m_KineBase.m_AxisRotTip[2],
              pRobot->m_KineBase.m_JointPG[2]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[2],
              pRobot->m_KineBase.m_AxisRotTip[3],
              pRobot->m_KineBase.m_JointPG[3]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[3],
              pRobot->m_KineBase.m_AxisRotTip[4],
              pRobot->m_KineBase.m_JointPG[4]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[4],
              pRobot->m_KineBase.m_AxisRotTip[5],
              pRobot->m_KineBase.m_JointPG[5]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[5],
              pRobot->m_KineBase.m_AxisRotTip[6],
              pRobot->m_KineBase.m_JointPG[6]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[6], pRobot->m_KineBase.m_Flange,
              pRobot->m_KineBase.m_FlangeTip);
    FX_PGMult(pRobot->m_KineBase.m_FlangeTip, pRobot->m_KineBase.m_Tool,
              pRobot->m_KineBase.m_TCP);
    FX_M44Copy(pRobot->m_KineBase.m_TCP, pgos);

    return FX_TRUE;
}

FX_BOOL FX_Robot_Kine_Piolt_NSPG(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                                 FX_DOUBLE pgos[4][4], Matrix3 nspg) {
    long i;
    FX_Robot *pRobot;
    FX_DOUBLE cosv, sinv;
    if (RobotSerial < 0 || RobotSerial >= MAX_RUN_ROBOT_NUM) {
...

    {
        ...
        zdir[0] = pRobot->m_KineBase.m_JointPG[6][0][3] -
                  pRobot->m_KineBase.m_JointPG[1][0][3];
        zdir[1] = pRobot->m_KineBase.m_JointPG[6][1][3] -
                  pRobot->m_KineBase.m_JointPG[1][1][3];
        zdir[2] = pRobot->m_KineBase.m_JointPG[6][2][3] -
                  pRobot->m_KineBase.m_JointPG[1][2][3];
    ...
    printf("EG:[% lf % lf % lf % lf \n %lf %lf %lf %lf \n %lf %lf %lf %lf \n "
           "%lf %lf %lf %lf \n",
           pgos[0][0], pgos[0][1], pgos[0][2], pgos[0][3], pgos[1][0],
           pgos[1][1], pgos[1][2], pgos[1][3], pgos[2][0], pgos[2][1],
           pgos[2][2], pgos[2][3], pgos[3][0], pgos[3][1], pgos[3][2],
           pgos[3][3]);
    return FX_TRUE;
}

FX_BOOL FX_Robot_Kine_DL(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                         FX_DOUBLE pgos[4][4]) {
    ...
    FX_PGMult(pRobot->m_KineBase.m_JointPG[4],
              pRobot->m_KineBase.m_AxisRotTip[5],
              pRobot->m_KineBase.m_JointPG[5]);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[5], pRobot->m_KineBase.m_Flange,
              pRobot->m_KineBase.m_FlangeTip);
    FX_PGMult(pRobot->m_KineBase.m_FlangeTip, pRobot->m_KineBase.m_Tool,
              pRobot->m_KineBase.m_TCP);
    FX_M44Copy(pRobot->m_KineBase.m_TCP, pgos);
...
FX_BOOL FX_Robot_Tool_Set(FX_INT32L RobotSerial, Matrix4 tool) {
    ...
    printf("EG:TOOL=[%lf %lf %lf %lf\n %lf %lf %lf %lf\n %lf %lf %lf %lf\n %lf "
           "%lf %lf %lf]\n",
           pRobot->m_KineBase.m_Tool[0][0], pRobot->m_KineBase.m_Tool[0][1],
           pRobot->m_KineBase.m_Tool[0][2], pRobot->m_KineBase.m_Tool[0][3],
           pRobot->m_KineBase.m_Tool[1][0], pRobot->m_KineBase.m_Tool[1][1],
           pRobot->m_KineBase.m_Tool[1][2], pRobot->m_KineBase.m_Tool[1][3],
           pRobot->m_KineBase.m_Tool[2][0], pRobot->m_KineBase.m_Tool[2][1],
           pRobot->m_KineBase.m_Tool[2][2], pRobot->m_KineBase.m_Tool[2][3],
           pRobot->m_KineBase.m_Tool[3][0], pRobot->m_KineBase.m_Tool[3][1],
           pRobot->m_KineBase.m_Tool[3][2], pRobot->m_KineBase.m_Tool[3][3]);
    return FX_TRUE;
}
...
FX_BOOL FX_Robot_Kine_FK(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                         Matrix4 pgos) {
    FX_Robot *pRobot;
    if (RobotSerial < 0 || RobotSerial >= MAX_RUN_ROBOT_NUM) {
        return FX_FALSE;
...
FX_BOOL FX_Robot_Kine_FK_NSP(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                             Matrix4 pgos, Matrix3 nspg) {
    FX_Robot *pRobot;
    if (RobotSerial < 0 || RobotSerial >= MAX_RUN_ROBOT_NUM) {
        return FX_FALSE;
    }
...
FX_BOOL FX_Jacb_ROT_7(FX_INT32L RobotSerial, FX_DOUBLE jcb[6][7]) {
    ...
    FX_Robot *pRobot = (FX_Robot *)&m_Robot[RobotSerial];
    for (i = 0; i < 7; i++) {
        Vm[0] = pRobot->m_KineBase.m_TCP[0][3] -
                pRobot->m_KineBase.m_JointPG[i][0][3];
        Vm[1] = pRobot->m_KineBase.m_TCP[1][3] -
                pRobot->m_KineBase.m_JointPG[i][1][3];
        Vm[2] = pRobot->m_KineBase.m_TCP[2][3] -
                pRobot->m_KineBase.m_JointPG[i][2][3];
    ...
    printf("EG:jcb[0]=[%lf %lf %lf %lf %lf %lf %lf]\n", jcb[0][0], jcb[0][1],
           jcb[0][2], jcb[0][3], jcb[0][4], jcb[0][5], jcb[0][6]);
    return FX_TRUE;
}
...
FX_BOOL FX_JacbAxis7(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                     FX_DOUBLE jcb[6][7]) {
    FX_Robot *pRobot = (FX_Robot *)&m_Robot[RobotSerial];
...
FX_BOOL FX_JacbAixs6(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                     FX_DOUBLE jcb[6][6]) {
    FX_Robot *pRobot = (FX_Robot *)&m_Robot[RobotSerial];
...
FX_BOOL FX_Robot_Kine_Jacb(FX_INT32L RobotSerial, FX_DOUBLE joints[7],
                           FX_Jacobi *jcb) {
...
FX_BOOL FX_Jacb_Dot_ROT_7(FX_INT32L RobotSerial, FX_DOUBLE jvel[7],
                          FX_DOUBLE jcb_dot[6][7]) {
    FX_INT32 i;
    FX_INT32 dof = 7;
    ...
    for (i = 0; i < dof - 1; i++) {
        p[i][0] = (pRobot->m_KineBase.m_JointPG[i + 1][0][3] -
                   pRobot->m_KineBase.m_JointPG[i][0][3]) *
                  0.001;
        p[i][1] = (pRobot->m_KineBase.m_JointPG[i + 1][1][3] -
                   pRobot->m_KineBase.m_JointPG[i][1][3]) *
                  0.001;
        p[i][2] = (pRobot->m_KineBase.m_JointPG[i + 1][2][3] -
                   pRobot->m_KineBase.m_JointPG[i][2][3]) *
                  0.001;
    }

    p[dof - 1][0] = (pRobot->m_KineBase.m_TCP[0][3] -
                     pRobot->m_KineBase.m_JointPG[dof - 1][0][3]) *
                    0.001;
    p[dof - 1][1] = (pRobot->m_KineBase.m_TCP[1][3] -
                     pRobot->m_KineBase.m_JointPG[dof - 1][1][3]) *
                    0.001;
    p[dof - 1][2] = (pRobot->m_KineBase.m_TCP[2][3] -
                     pRobot->m_KineBase.m_JointPG[dof - 1][2][3]) *
                    0.001;
    ...
    printf("EG:jcb_dot[0]=%lf %lf %lf %lf %lf %lf %lf\n", jcb_dot[0][0],
           jcb_dot[0][1], jcb_dot[0][2], jcb_dot[0][3], jcb_dot[0][4],
           jcb_dot[0][5], jcb_dot[0][6]);
    return FX_TRUE;
}

FX_BOOL FX_Jacb_Dot_ROT_6(FX_INT32L RobotSerial, FX_DOUBLE jvel[7],
                          FX_DOUBLE jcb_dot[6][6]) {
    FX_INT32 i;
    FX_INT32 dof = 6;
    FX_DOUBLE e[8][3] = {{0}, {0}, {0}, {0}, {0}, {0}, {0}, {0}};
    FX_DOUBLE w[8][3] = {{0}, {0}, {0}, {0}, {0}, {0}, {0}, {0}};
...
FX_BOOL FX_SolveTrange2D(FX_DOUBLE x_pan, FX_DOUBLE r1x, FX_DOUBLE r1y,
                         FX_DOUBLE r2x, FX_DOUBLE r2y, FX_DOUBLE ret_xy[2]) {
    FX_DOUBLE k1;
    FX_DOUBLE k2;
...
FX_BOOL FX_SolveJ123ZNYZ(NSPBase *pnspbase, FX_DOUBLE rot_angle,
                         FX_DOUBLE ref[7], FX_DOUBLE ret_j[7],
                         FX_BOOL *is_degen, FX_DOUBLE dgr) {
    FX_DOUBLE m[3][3];
...
FX_BOOL FX_SolveJ567ZNYZ(NSPBase *pnspbase, FX_DOUBLE rot_angle,
                         FX_DOUBLE ref[7], FX_DOUBLE ret_j[7],
                         FX_BOOL *is_degen, FX_DOUBLE dgr) {
    FX_DOUBLE m1[3][3];
    FX_DOUBLE m2[3][3];
    ...
    *is_degen = FX_TRUE;

    {
        FX_DOUBLE err1;
        FX_DOUBLE err2;
        FX_MatRotAxis(pnspbase->rot_axis, rot_angle + 1, pnspbase->j567Base,
                      m1);
        FX_M33Trans(m1, m2);
...
FX_BOOL FX_SolveJ567ZNYX(NSPBase *pnspbase, FX_DOUBLE rot_angle,
                         FX_DOUBLE ref[7], FX_DOUBLE ret_j[7],
                         FX_BOOL *is_degen, FX_DOUBLE dgr) {
...
FX_BOOL FX_InvKine_Pilot(FX_INT32L RobotSerial,
    ...
    FX_MMM44(solve_para->m_Input_IK_TargetTCP, pRobot->m_KineBase.m_InvTool,
             m_flan);
...

    {
        ...
        if (j4type1 == 0 || j4type2 == 0 || j4type1 == j4type2) {
            ...
            if (FX_Fabs(Jv3[0]) < 0.02 || FX_Fabs(Jv3[1]) < 0.02) {
                ...
                solve_para->m_Output_RetJoint[0] =
                    solve_para->m_Input_IK_RefJoint[0];
                solve_para->m_Output_RetJoint[1] =
                    solve_para->m_Input_IK_RefJoint[1];
                solve_para->m_Output_RetJoint[2] =
                    solve_para->m_Input_IK_RefJoint[2];
                solve_para->m_Output_RetJoint[3] =
                    solve_para->m_Input_IK_RefJoint[3];
                solve_para->m_Output_RetJoint[4] =
                    solve_para->m_Input_IK_RefJoint[4];
                solve_para->m_Output_RetJoint[5] =
                    solve_para->m_Input_IK_RefJoint[5];
                solve_para->m_Output_RetJoint[6] =
                    solve_para->m_Input_IK_RefJoint[6];
    ...
    if (j4DegTag == FX_TRUE) {
...

        {
            ...
            { // solve 123
                ...
                if (FX_Matrix2ZYZ(A_M123, solve123) == FX_TRUE) {
                    ...
                    err1 = FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[0],
                                            &j1_a) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[1],
                                            &j2_a) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[2],
                                            &j3_a);
                    err2 = FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[0],
                                            &j1_b) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[1],
                                            &j2_b) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[2],
            ...
            { // solve 456
                ...
                if (SPC->m_IsCross == FX_FALSE) {
                    if (FX_Matrix2ZYZ(A_M567_tran, solve567) == FX_TRUE) {
                        ...
                        err1 =
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[4],
                                             &j5_a) +
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[5],
                                             &j6_a) +
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[6],
                                             &j7_a);
                        err2 =
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[4],
                                             &j5_b) +
                        ...
                        err1 =
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[0],
                                             &j5_a) +
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[1],
                                             &j6_a) +
                            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[2],
                    ...
                    FX_DOUBLE j7_a = solve567[2];
                    FX_DOUBLE err1;
                    err1 = FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[4],
                                            &j5_a) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[5],
                                            &j6_a) +
                           FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[6],
        ...
        if (1) /// solve A Case
        {
            ...
            angb += SPC->Ang2;
            {
                ...
                if (FX_SolveTrange2D(ablen, v1x, v1y, v2x, v2y, pt) ==
                    FX_FALSE) {
            ...
            FX_DOUBLE det123x = FX_Sqrt(vect123_x[0] * vect123_x[0] +
                                        vect123_x[1] * vect123_x[1] +
                                        vect123_x[2] * vect123_x[2]);
            FX_DOUBLE det123y = FX_Sqrt(vect123_y[0] * vect123_y[0] +
                                        vect123_y[1] * vect123_y[1] +
                                        vect123_y[2] * vect123_y[2]);
            ...
            det123x = FX_Sqrt(vect567_x[0] * vect567_x[0] +
                              vect567_x[1] * vect567_x[1] +
                              vect567_x[2] * vect567_x[2]);
            det123y = FX_Sqrt(vect567_y[0] * vect567_y[0] +
                              vect567_y[1] * vect567_y[1] +
                              vect567_y[2] * vect567_y[2]);
        ...
        min_err4_A =
            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[3], &JV4_A);
        min_err4_B =
            FX_MinDif_Circle(solve_para->m_Input_IK_RefJoint[3], &JV4_B);

        if (min_err123_A + min_err4_A + min_err567_A <
            min_err123_B + min_err4_B + min_err567_B) {
            solve_para->m_Output_RetJoint[0] = min_A_123[0];
            solve_para->m_Output_RetJoint[1] = min_A_123[1];
            solve_para->m_Output_RetJoint[2] = min_A_123[2];
...
FX_BOOL FX_InvKine_Pilot_SRS(FX_INT32L RobotSerial,
                             FX_InvKineSolvePara *solve_para) {
    FX_Robot *pRobot;
    FX_KineSPC_Pilot *SPC;
    NSPBase *NSP;
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (solve_para->m_Output_RetJoint[i] <
            solve_para->m_Output_RunLmtN[i]) {
            solve_para->m_Output_IsJntExd = FX_TRUE;
            solve_para->m_Output_JntExdTags[i] = FX_TRUE;
            exdabs = FX_Fabs(solve_para->m_Output_RetJoint[i] -
                             solve_para->m_Output_RunLmtN[i]);
            if (solve_para->m_Output_JntExdABS < exdabs) {
                solve_para->m_Output_JntExdABS = exdabs;
            }
        } else if (solve_para->m_Output_RetJoint[i] >
                   solve_para->m_Output_RunLmtP[i]) {
            solve_para->m_Output_IsJntExd = FX_TRUE;
            solve_para->m_Output_JntExdTags[i] = FX_TRUE;
            exdabs = FX_Fabs(solve_para->m_Output_RetJoint[i] -
                             solve_para->m_Output_RunLmtP[i]);
    ...
    printf("EG:ret_J=[%lf %lf %lf %lf %lf %lf %lf]\n",
           solve_para->m_Output_RetJoint[0], solve_para->m_Output_RetJoint[1],
           solve_para->m_Output_RetJoint[2], solve_para->m_Output_RetJoint[3],
           solve_para->m_Output_RetJoint[4], solve_para->m_Output_RetJoint[5],
           solve_para->m_Output_RetJoint[6]);
    return FX_TRUE;
}

FX_BOOL FX_InvKine_Pilot_CCS(FX_INT32L RobotSerial,
                             FX_InvKineSolvePara *solve_para) {
    FX_Robot *pRobot;
    FX_KineSPC_Pilot *SPC;
    NSPBase *NSP;
    ...
    if (solve_para->m_Output_RetJoint[5] > 1) {
        ...
        P = SPC->lmtj67_pp[0] * J6 * J6 + SPC->lmtj67_pp[1] * J6 +
            SPC->lmtj67_pp[2];
        if (solve_para->m_Output_RunLmtP[6] > P) {
            solve_para->m_Output_RunLmtP[6] = P;
        }
        printf("j7=%lf line:%lf\n", solve_para->m_Output_RunLmtP[6],
               solve_para->m_Output_RetJoint[6]);
        if (solve_para->m_Output_RetJoint[6] >
            solve_para->m_Output_RunLmtP[6]) {
            solve_para->m_Output_IsJntExd = FX_TRUE;
            solve_para->m_Output_JntExdTags[6] = FX_TRUE;

            exdabs = FX_Fabs(solve_para->m_Output_RetJoint[6] -
                             solve_para->m_Output_RunLmtP[6]);
            if (solve_para->m_Output_JntExdABS < exdabs) {
                solve_para->m_Output_JntExdABS = exdabs;
            }
        ...
        N = SPC->lmtj67_pn[0] * J6 * J6 + SPC->lmtj67_pn[1] * J6 +
            SPC->lmtj67_pn[2];
    ...
    if (solve_para->m_Output_RetJoint[5] < -1) {
        ...
        P = SPC->lmtj67_np[0] * J6 * J6 + SPC->lmtj67_np[1] * J6 +
            SPC->lmtj67_np[2];
        if (solve_para->m_Output_RunLmtP[6] > P) {
            solve_para->m_Output_RunLmtP[6] = P;
        }
        ...
        N = SPC->lmtj67_nn[0] * J6 * J6 + SPC->lmtj67_nn[1] * J6 +
            SPC->lmtj67_nn[2];
        if (solve_para->m_Output_RunLmtN[6] < N) {
            solve_para->m_Output_RunLmtN[6] = N;
        }
...
FX_BOOL FX_Robot_Kine_IK(FX_INT32L RobotSerial,
                         FX_InvKineSolvePara *solve_para) {
...
FX_BOOL FX_InvKineNSP_Pilot(FX_INT32L RobotSerial,
                            FX_InvKineSolvePara *solve_para) {
    FX_Robot *pRobot;
    FX_KineSPC_Pilot *SPC;
    NSPBase *NSP;
    ...
    if (FX_SolveJ123ZNYZ(
            NSP, solve_para->m_Input_ZSP_Angle, solve_para->m_Input_IK_RefJoint,
            solve_para->m_Output_RetJoint, &(solve_para->m_Output_IsDeg[1]),
            solve_para->m_DGR1) == FX_FALSE) {
        return FX_FALSE;
    }
    ...
    if (NSP->m_IsCorss == FX_TRUE) {
        FX_INT32L i;
        if (FX_SolveJ567ZNYX(NSP, solve_para->m_Input_ZSP_Angle,
                             solve_para->m_Input_IK_RefJoint,
                             solve_para->m_Output_RetJoint,
                             &(solve_para->m_Output_IsDeg[5]),
                             solve_para->m_DGR2) == FX_FALSE) {
            return FX_FALSE;
        }
        ...
        if (FX_SolveJ567ZNYZ(NSP, solve_para->m_Input_ZSP_Angle,
                             solve_para->m_Input_IK_RefJoint,
                             solve_para->m_Output_RetJoint,
                             &(solve_para->m_Output_IsDeg[5]),
                             solve_para->m_DGR2) == FX_FALSE) {
...
FX_BOOL FX_Robot_Kine_IK_NSP(FX_INT32L RobotSerial,
                             FX_InvKineSolvePara *solve_para) {
...
FX_BOOL FX_Robot_Kine_W(FX_INT32L RobotSerial, Vect7 jcur, Vect7 retW) {
    ...
    FX_VectNorm(zdir);
    FX_PGMult(pRobot->m_KineBase.m_JointPG[3],
              pRobot->m_KineBase.m_AxisRotBase[4], Base);
...
FX_BOOL FX_Robot_Kine_Err(FX_INT32L RobotSerial, Vect7 angleTar, Vect7 angleCur,
                          Vect3 ErrXYZ, Vect3 ErrGes, FX_DOUBLE *errNSP,
                          Vect7 nspW) {
    FX_Robot *pRobot;
    Matrix4 pgt;
    Matrix4 pgc;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
FX_BOOL FX_Robot_PLN_MOVL(FX_INT32L RobotSerial, Vect6 Start_XYZABC,
                          Vect6 End_XYZABC, Vect7 Ref_Joints, FX_DOUBLE Vel,
                          FX_DOUBLE ACC, FX_INT8 *OutPutPath) {
    ...
    AxisPln_OnMovL(Spln, RobotSerial, start_pos, end_pos, refJ, Vel, ACC, jerk,
                   OutPutPath);
...
FX_BOOL FX_Robot_PLN_MOVL_KeepJ(FX_INT32L RobotSerial, Vect7 startjoints,
                                Vect7 stopjoints, FX_DOUBLE vel,
                                FX_CHAR *OutPutPath) {
    ...
    FXSpln Spln = AxisPln_Create();
    AxisPln_OnMovL_KeepJ(Spln, RobotSerial, start_pos, end_pos, vel,
                         OutPutPath);
...
FX_INT32 FX_Robot_Iden_LoadDyn(FX_INT32 TYPE, FX_CHAR *path, FX_DOUBLE *mass,
                               Vect3 mr, Vect6 I) {
