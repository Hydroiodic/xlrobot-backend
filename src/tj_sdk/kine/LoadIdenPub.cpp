...
static bool strSpliteSRS(const string &str, vector<vector<string>> &loadData) {
    ...
    while (getline(ss, tem, ',')) {
        ...
        if (4 == dataPos || 6 == dataPos || 7 == dataPos || 11 == dataPos ||
            13 == dataPos || 14 == dataPos) {
...
LoadIdenErrCode OnCalLoadTor_7_MarvinSRS(LoadTor *tor, int isNoLoad,
                                         const char *userPath) {
    if (tor == NULL) {
        printf("tor is null!\n");
    ...
    if (!ifsConFile.is_open()) {
        cout << "ERROR: load cfg file failed, pls check "
                "file<LoadIdenCfg_Marvin_SRS.txt>"
             << endl;
        return LOAD_IDEN_OpenCfgFileErr;
    }
    string strCfg;
    if (!getline(ifsConFile, strCfg)) {
        cout << "Error: load config failed, type-1, pls contact supplier for "
                "help."
             << endl;
        return LOAD_IDEN_OpenCfgFileErr;
    }
    ...
    if (vecCon.size() != 17) {
        cout << "Error: load config failed, type-2, pls contact supplier for "
                "help."
             << endl;
        return LOAD_IDEN_OpenCfgFileErr;
    }
    ...
    for (i = dataTagPos; i < LoadDataNum - 1; i++) {
        if (torTag[6] == stod(vecLoadData.at(DataTag).at(i))) {
            while (i < LoadDataNum - 1) {
                ...
                if (((dtemp2 - dtemp) * sampleCy < trajCondation[8]) &&
                    (dtemp2 - dtemp) * sampleCy > trajCondation[9]) {
                    break;
                }
                i++;
            }
    ...
    dtemp = 0;
    if (dataFlagCount != 7) {
        cout << "ERROR: insufficient effective data, pls check LoadData.csv or "
                "NoLoadData.csv"
             << endl;
    ...
    if (1 == isNoLoad) {
        ...
        for (int i = 0; i < vecCon.size() - 1; i++) {
            if (!(ofsConFile << vecCon.at(i) << ",")) {
                cout << "ERROR: pls check ./LoadData/CfgFileLoadIdenCfg"
                     << endl;
                return LOAD_IDEN_OpenCfgFileErr;
            }
        }
...
LoadIdenErrCode OnCalLoadDynamic_7_MarvinSRS(LoadDynamic *loadDyn,
                                             const char *userPath) {
...
static bool strSpliteCCS(const string &str, vector<vector<string>> &loadData) {
    ...
    while (getline(ss, tem, ',')) {
        dataPos++;
        if (4 == dataPos || 5 == dataPos || 6 == dataPos || 7 == dataPos ||
            11 == dataPos || 12 == dataPos || 13 == dataPos || 14 == dataPos) {
            deqItem.push_back(tem);
        }
        if (15 == dataPos) {
            deqItem.push_front(tem);
...
LoadIdenErrCode OnCalLoadTor_7_MarvinCCS(LoadTor *tor, int isNoLoad,
                                         const char *userPath) {
    if (tor == NULL) {
        printf("tor is null!\n");
        return LOAD_IDEN_CalErr;
    ...
    ifstream ifsConFile(strIdenCfgName, ios::in);
    if (!ifsConFile.is_open()) {
        cout << "ERROR: open file failed, pls check :LoadIdenCfg_MarvinCCS.txt"
             << endl;
        return LOAD_IDEN_OpenCfgFileErr;
    }
    string strCfg;
    ...
    if (1 == isNoLoad) {
        ...
        if (!ofsConFile.is_open()) {
            cout << "ERROR: open cfg file failed, pls check LoadIdenCfg"
                 << endl;
            return LOAD_IDEN_DataSmpErr;
        }
        for (int i = 0; i < vecCon.size() - 1; i++) {
            if (!(ofsConFile << vecCon.at(i) << ",")) {
                cout << "ERROR: write cfg file failed, pls check LoadIdenCfg"
                     << endl;
                return LOAD_IDEN_DataSmpErr;
            }
        }
...
LoadIdenErrCode OnCalLoadDynamic_7_MarvinCCS(LoadDynamic *loadDyn,
                                             const char *userPath) {
    cout << "Calculating load data..." << endl;
    LoadTor loadTor;
    memset(&loadTor, 0, sizeof(loadTor));
    ...
    if (retCode != LOAD_IDEN_NoErr) {
        cout << "ERROR: Calculated load identification parameters failed!"
             << endl;
        return retCode;
    }
...
LoadIdenErrCode OnCalLoadDyn(LoadDynamicPara *DynPara, FX_INT32 RobotType,
                             const FX_CHAR *UserPath) {
    DynPara->m = 0;
    DynPara->r[0] = 0;
    DynPara->r[1] = 0;
    DynPara->r[2] = 0;
