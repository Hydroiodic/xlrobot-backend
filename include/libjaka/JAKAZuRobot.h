...
class DLLEXPORT_API JAKAZuRobot {
  public:
     ...
     * @param step_num
     * 步数，表示发送指令的步数，默认1步。比如调用周期无法保证8ms，则可以设置为更大值，比如2，则调用周期可允许为16ms。
     * 				主要用于客户端实时性不好的场景，容许更大的抖动。或处理周期需要更长的情况。
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t edg_servo_j(unsigned char robot_index, const JointValue *joint_pos,
                        MoveMode move_mode, unsigned int step_num = 1);
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t edg_servo_p(unsigned char robot_index,
                        const CartesianPose *cartesian_pose, MoveMode move_mode,
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t edg_get_stat(unsigned char robot_index, JointValue *joint_pos,
                         CartesianPose *cartesian_pose);

    /**
     * @brief
     * 获取机器人状态详细信息，注意此接口获取的为缓存数据，刷新周期约为8ms
     ...
     * @param keys
     * 末端FREE按钮状态，0表示未按下，1表示按下。分别代表两个机器人的
     * @return ERR_SUCC 成功 其他失败
     */
     ...
     * @param cmd_index
     * 指令索引，用于标识指令的唯一性，如果为nullptr，则内部默认自增
     * @return ERR_SUCC 成功 其他失败
     */
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t kine_inverse(int robot_id, const JointValue *ref_pos,
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t kine_forward(int robot_id, const JointValue *joint_pos,
                         CartesianPose *cartesian_pose);
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t servo_move_use_joint_NLF(double max_vr, double max_ar,
                                     double max_jr);
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t servo_move_use_carte_NLF(double max_vp, double max_ap,
                                     double max_jp, double max_vr,
                                     double max_ar, double max_jr);
     ...
     * @return ERR_SUCC 成功 其他失败
     */
    errno_t servo_move_use_joint_MMF(int max_buf, double kp, double kv,
                                     double ka);
    ...
    /// @return
    errno_t robot_run_multi_movj(int robot_id, const MoveMode *move_mode,
                                 BOOL is_block, const JointValue *joint_pos,
    ...
    /// @param acc 两个机器人的加速度指令
    /// @return
    errno_t robot_run_multi_movl(int robot_id, const MoveMode *move_mode,
                                 BOOL is_block, const CartesianPose *end_pos,
                                 const double *vel, const double *acc);
    ...
    /// @param deadzone_percent
    /// 传感器灵敏度死区占默认最大死区的百分比，数据范围0-1
    /// @return
    errno_t robot_set_ftsensor_deadzone(int robot_id, int sensor_id,
                                        const double *deadzone_percent);

    errno_t robot_get_ftsensor_deadzone(int robot_id, int sensor_id,
    ...
    /// @param ftUser
    /// 力控刚度，需为正数，请勿设置为0，一般建议x,y,z设置10以上的数值，mx,my,mz设置1以上的数值
    /// @param ftReboundFK 回弹弹力系数，需为正数，可以设置为0
    /// @param ftConstant 目标力，请勿设置超出机器人承受范围的目标力
    /// @return
    errno_t robot_set_cst_ftconfig(int robot_id, int axis, int enable,
                                   double ftUser, double ftReboundFK,
                                   double ftConstant);

    errno_t robot_get_cst_ftconfig(int robot_id, RobotAdmitCtrl *config);

    /// @brief
    /// 开启恒力柔顺控制接口，恒力柔顺控制可以配合直线或关节运动以及笛卡尔伺服运动模式使用，不可以配合拖拽模式或手动示教JOG使用
    /// @param robot_id 机器人ID 接受LEFT(0) RIGHT(1) DUAL(-1)
    /// @return
    errno_t robot_enable_force_control(int robot_id);

    /// @brief
    /// 关闭恒力柔顺控制接口，注意关闭力控将使力控立即失效但不会退出力控模式
    ...
    /// @brief
    /// 传感器校零接口，注意校零需要约1秒的时间，期间无法开启力控，固建议调用此接口后等待1秒。
    ...
    /// @brief
    /// 设置传感器负载接口，注意传感器负载与机器人负载可以不同（也可以相同），需要单独设置
    /// @param robot_id 机器人ID 接受LEFT(0) RIGHT(1) DUAL(-1)
    /// @param sensor_id 传感器ID 数据范围1-199
    /// @param payload 负载数据，单位kg，mm
    /// @return
    errno_t robot_set_ftsensor_payload(int robot_id, int sensor_id,
                                       PayLoad payload);

    errno_t robot_get_ftsensor_payload(int robot_id, int sensor_id,
                                       PayLoad *payload);
    ...
    /// @return
    errno_t robot_set_ftsensor_filter(int robot_id, int sensor_id,
                                      double filter);

    errno_t robot_get_ftsensor_filter(int robot_id, int sensor_id,
                                      double *filter);

    /// @brief
    /// 获取力控模式状态，注意当调用disable_force_control后但机器人尚未完成当前运动期间力控模式仍被视作开启状态
    /// @param robot_id 机器人ID 接受LEFT(0) RIGHT(1)
    ...
    /// @return
    errno_t robot_get_ftsensor_stat(int robot_id, int sensor_id, int *status,
                                    int *errcode, double *ft_original,
                                    double *ft_actual);
     ...
     * @param robot_id 机器人ID 接受LEFT(0) RIGHT(1)
     * @param joint_id 关节ID
     * @param pos_kp 位置环比例增益,推荐30，
     * 范围0~100.参数越大，位置环越刚，该参数设置过大会导致机器人出现振动，建议在默认值附近调整
     * @param vel_kp 速度环比列增益,推荐50，
     * 范围0~200.参数越大，速度环越刚，该参数设置过大会导致机器人出现振动，建议在默认值附近调整
     * @param vel_ti 速度环积分常数,推荐2000，
     * 范围500~16777216.参数越大，积分作用越弱，该参数设置过小会导致速度超调甚至失稳，建议在默认值附近调整
     * @note
     * 当且仅当开启controlloop时才有效。关闭controlloop后，参数会被重置为内部默认值
     * @note 仅支持3.0.3_ZY1及其以上版本
     */
    errno_t set_joint_controlloop(int robot_id, int joint_id, uint32_t pos_kp,
                                  uint32_t vel_kp, uint32_t vel_ti);
