...
bool CAxisPln::OnPln(double start_pos, double end_pos, double vel, double acc,
                     double jerk, CPointSet *ret) {
    ret->OnInit(PotT_2d);
    ret->OnEmpty();
...
bool CAxisPln::OnPlnAcc(double start_pos, double end_pos, double vel,
                        double acc, double jerk, CPointSet *ret) {
    ret->OnInit(PotT_2d);
    ret->OnEmpty();
    double s = fabs(start_pos - end_pos);
    ...
    //	pathCon[0] = pathLast[0] + pathLast[1] * t + 0.5 * fabs(acc) * (-sig) *
    //t * t; 	if (i == 1) { 		pathEnd[0] = pathCon[0]; 		pathEnd[1] = pathCon[1];
    //		continue;
    //	}
    ...
    for (int i = 1; i < last; i++) {
        ...
        pathCon[1] = pathLast[1] + fabs(acc) * (-sig) * t;
        pathCon[0] =
            pathLast[0] + pathLast[1] * t + 0.5 * fabs(acc) * (-sig) * t * t;
        if (i == 1) {
            pathEnd[0] = pathCon[0];
            pathEnd[1] = pathCon[1];
            continue;
    ...
    pathCon[0]); pathLast[1] = pathCon[1]; printf("pathCon[1]=%f-----------\n",
    pathCon[1]);*/
    double trajCon[2] = {0};
    for (int i = 1; i < last; i++) {
        double t = i * 0.002;
        trajCon[1] =
            pathCon[1] +
            fabs(acc) *
                (sig)*t; // printf("pathCon[1]=%f-----------\n", pathCon[1]);
        trajCon[0] =
            pathCon[0] + pathCon[1] * t + 0.5 * fabs(acc) * (sig)*t * t;
        if (trajCon[1] > 0) {
            break;
        }
...
bool CAxisPln::OnPlnAccNew(double start_pos, double end_pos, double vel,
                           double acc, double jerk, CPointSet *ret,
                           CPointSet *ps) {
    // ��������-����-����-����-����
    ret->OnInit(PotT_2d);
...
bool CAxisPln::OnPlnAccSimple(double start_pos, double vel, double accmax,
                              CPointSet *axis) {
    axis->OnInit(PotT_2d);
    axis->OnEmpty();
    double x0 = start_pos;
...
bool CAxisPln::OnMovL(long RobotSetial, double ref_joints[7],
                      double start_pos[6], double end_pos[6], double vel,
                      double acc, double jerk, char *path) {
...
bool CAxisPln::OnMovL_KeepJ(long RobotSerial, double startjoints[7],
                            double stopjoints[7], double vel, char *path) {
    CPointSet retJoints;
...
bool CAxisPln::OnMovJ(long RobotSetial, double start_joint[7],
                      double end_joint[7], double vel, double acc, double jerk,
                      char *path) {
