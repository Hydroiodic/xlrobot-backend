...
FX_VOID SVD_CALP(FX_DOUBLE a[], FX_DOUBLE e[], FX_DOUBLE s[], FX_DOUBLE v[],
                 FX_INT32L m, FX_INT32L n) {
    FX_INT32L i, j, p, q;
    FX_DOUBLE d;
    if (m >= n)
        i = n;
...
FX_INT32L SVD_UAV(FX_DOUBLE a[], FX_INT32L m, FX_INT32L n, FX_DOUBLE u[],
                  FX_DOUBLE v[], FX_DOUBLE eps, FX_INT32L ka) {
        ...
        if (kk == mm - 1) {
            ...
            if (ks == kk) {
                ...
                for (i = kk; i <= mm - 1; i++) {
                    ...
                    if ((FX_Fabs(cs[0] - 1.0) > FXARM_EPS) ||
                        (FX_Fabs(cs[1]) > FXARM_EPS))
                        for (j = 1; j <= n; j++) {
                            ix = (j - 1) * n + i - 1;
                            iy = (j - 1) * n + i;
...
FX_VOID FX_PGPointMap(Matrix4 a_to_b, Vect3 in_a, Vect3 ret_in_b) {
    ret_in_b[0] = a_to_b[0][3] + a_to_b[0][0] * in_a[0] +
                  a_to_b[0][1] * in_a[1] + a_to_b[0][2] * in_a[2];
    ret_in_b[1] = a_to_b[1][3] + a_to_b[1][0] * in_a[0] +
                  a_to_b[1][1] * in_a[1] + a_to_b[1][2] * in_a[2];
    ret_in_b[2] = a_to_b[2][3] + a_to_b[2][0] * in_a[0] +
                  a_to_b[2][1] * in_a[1] + a_to_b[2][2] * in_a[2];
}

FX_VOID FX_PGVectMap(Matrix4 a_to_b, Vect3 in_a, Vect3 ret_in_b) {
    ret_in_b[0] = a_to_b[0][0] * in_a[0] + a_to_b[0][1] * in_a[1] +
                  a_to_b[0][2] * in_a[2];
    ret_in_b[1] = a_to_b[1][0] * in_a[0] + a_to_b[1][1] * in_a[1] +
                  a_to_b[1][2] * in_a[2];
    ret_in_b[2] = a_to_b[2][0] * in_a[0] + a_to_b[2][1] * in_a[1] +
                  a_to_b[2][2] * in_a[2];
}

FX_VOID FX_PGVectMapInv(Matrix4 a_to_b, Vect3 ret_in_a, Vect3 in_b) {
    ret_in_a[0] = a_to_b[0][0] * in_b[0] + a_to_b[1][0] * in_b[1] +
                  a_to_b[2][0] * in_b[2];
    ret_in_a[1] = a_to_b[0][1] * in_b[0] + a_to_b[1][1] * in_b[1] +
                  a_to_b[2][1] * in_b[2];
    ret_in_a[2] = a_to_b[0][2] * in_b[0] + a_to_b[1][2] * in_b[1] +
                  a_to_b[2][2] * in_b[2];
}
...
FX_VOID FX_PGMult(Matrix4 L, Matrix4 R, Matrix4 Result) {
    ...
    Result[0][3] =
        L[0][0] * R[0][3] + L[0][1] * R[1][3] + L[0][2] * R[2][3] + L[0][3];
    Result[1][3] =
        L[1][0] * R[0][3] + L[1][1] * R[1][3] + L[1][2] * R[2][3] + L[1][3];
    Result[2][3] =
        L[2][0] * R[0][3] + L[2][1] * R[1][3] + L[2][2] * R[2][3] + L[2][3];
...
FX_VOID FX_PGMatrixInv(Matrix4 pg, Matrix4 pginv) {
    ...
    pginv[0][3] =
        -(pginv[0][0] * p[0] + pginv[0][1] * p[1] + pginv[0][2] * p[2]);
    pginv[1][3] =
        -(pginv[1][0] * p[0] + pginv[1][1] * p[1] + pginv[1][2] * p[2]);
    pginv[2][3] =
        -(pginv[2][0] * p[0] + pginv[2][1] * p[1] + pginv[2][2] * p[2]);
}

FX_VOID FX_MatRotAxis(Vect3 rot_dir, FX_DOUBLE rot_angle, Matrix3 m,
                      Matrix3 m_roted) {
    FX_DOUBLE r[3][3];
    FX_DOUBLE sinv;
    FX_DOUBLE cosv;
...
FX_BOOL FX_Matrix2ZYX_DGR(Matrix3 m, FX_DOUBLE dgr, Vect3 ret) {
    FX_DOUBLE cb;
    ret[1] =
        FX_ATan2(-m[2][0], FX_Sqrt(m[0][0] * m[0][0] + m[1][0] * m[1][0])) *
        FXARM_R2D;
    cb = FX_COS_DEG(ret[1]);
...
FX_BOOL generalized_eig(double eps, double A[6][6], double B[6][6],
                        double V[6][6], double D[6]) {
    FX_INT32 i, j, tret;
    Matrix6 G = {0};
    Matrix6 GInv = {0};
    Matrix6 GInvT = {0};
...
FX_BOOL FX_QuaternionNorm(Quaternion Q) {
    FX_DOUBLE qq =
        FX_Sqrt(Q[0] * Q[0] + Q[1] * Q[1] + Q[2] * Q[2] + Q[3] * Q[3]);
    if (qq <= FXARM_EPS) {
        return FX_FALSE;
    }
    Q[0] /= qq;
...
FX_VOID FX_QuaternionSlerp(Quaternion Q_from, Quaternion Q_to, FX_DOUBLE ratio,
                           Quaternion Q_ret) {
    FX_DOUBLE omega, cosom, sinom, scale0, scale1;
    cosom = Q_from[0] * Q_to[0] + Q_from[1] * Q_to[1] + Q_from[2] * Q_to[2] +
            Q_from[3] * Q_to[3];
