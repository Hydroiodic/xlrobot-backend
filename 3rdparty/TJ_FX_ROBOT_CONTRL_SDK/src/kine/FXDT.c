...
FX_BOOL FXVDT_OnInit(FXVDT *obj, FX_DOUBLE step_interval, FX_DOUBLE max_vel,
                     FX_DOUBLE max_acc) {
    obj->m_InitTag = FX_FALSE;
    obj->m_o_lmt = FX_FALSE;
    if (step_interval < 1.0 || step_interval > 1000.0 || max_vel < 0.1 ||
        max_vel > 5000.0 || max_acc < 0.01 || max_acc > 50000.0) {
        return FX_FALSE;
    }
...
FX_BOOL FXVDT_OnSetRunTimeLmt(FXVDT *obj, FX_DOUBLE vel_lmt,
                              FX_DOUBLE acc_lmt) {
    if (obj->m_InitTag == FX_FALSE) {
        return FX_FALSE;
    }
    if (FX_Fabs(vel_lmt - obj->m_v_lmt) < 0.1 &&
        FX_Fabs(obj->m_r - acc_lmt) < 0.1) {
        return FX_TRUE;
    }
    { // Update acc_lmt
        obj->m_r = acc_lmt;
...
FX_BOOL FXNSyncDTS_OnInit(FX_DTS *obj, FX_INT32L axis_num,
                          FX_DOUBLE step_interval, FX_DOUBLE max_vel[8],
                          FX_DOUBLE max_acc[8], FX_DOUBLE max_jrk[8]) {
    FX_INT32L i;
    obj->m_InitTag = FX_FALSE;
    obj->m_AxisNum = 0;
    ...
    for (i = 0; i < axis_num; i++) {
        ...
        if (FXVDT_OnInit(&obj->m_tds[i], step_interval, max_vel[i],
                         max_acc[i]) == FX_FALSE) {
            return FX_FALSE;
        }
...
FX_BOOL FXNSyncDTS_OnSetRunTimeLmt(FX_DTS *obj, FX_DOUBLE vel_lmt[8],
                                   FX_DOUBLE acc_lmt[8]) {
    FX_INT32L i;
    if (obj->m_InitTag == FX_FALSE) {
        return FX_FALSE;
    ...
    for (i = 0; i < obj->m_AxisNum; i++) {
        if (FXVDT_OnSetRunTimeLmt(&obj->m_tds[i], vel_lmt[i], acc_lmt[i]) ==
            FX_FALSE) {
            return FX_FALSE;
        }
    }
...
FX_BOOL FXNSyncDTS_OnGetCurPVA(FX_DTS *obj, FX_DOUBLE target[8],
                               FX_DOUBLE pos[8], FX_DOUBLE vel[8],
                               FX_DOUBLE acc[8]) {
    FX_INT32L i;
    if (obj->m_InitTag == FX_FALSE) {
        return FX_FALSE;
...
FX_BOOL FXNSyncDTS_OnGetCurPVA_U(FX_DTS *obj, FX_DOUBLE pos[8],
                                 FX_DOUBLE vel[8], FX_DOUBLE acc[8]) {
    FX_INT32L i;
    if (obj->m_InitTag == FX_FALSE) {
        return FX_FALSE;
    }
...
FX_BOOL FXNSyncDTS_OnDT(FX_DTS *obj, FX_DOUBLE target_pos[8],
                        FX_DOUBLE ret_pos[8]) {
    if (obj->m_InitTag == FX_FALSE) {
        return FX_FALSE;
    }
    ...
    for (i = 0; i < obj->m_AxisNum; i++) {
        ...
        FX_DOUBLE cur_vel =
            (ret_pos[i] - obj->m_lastPos[i]) / obj->m_tds[i].m_ts;
