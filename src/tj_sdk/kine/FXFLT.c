...
FX_BOOL FXAVGF_OnInit(FXAVGF *obj, FX_INT32L axis_num, FX_INT32L filter_size,
                      FX_DOUBLE ts) {
    FX_INT32L i;
    if (axis_num <= 0 || axis_num > 8 || filter_size <= 0 ||
        filter_size > 100) {
        return FX_FALSE;
    }
...
FX_BOOL FXAVGF_OnFilt(FXAVGF *obj, FX_DOUBLE data[7], FX_DOUBLE ret_data[7],
                      FX_DOUBLE ret_vel[7]) {
    FX_INT32L i, j;
    if (obj->m_init_tag == FX_FALSE) {
        return FX_FALSE;
...
FX_BOOL FXSMV_OnInit(FX_SmoothValue *obj, FX_DOUBLE current_value,
                     FX_DOUBLE new_value_ratio, FX_DOUBLE max_step,
                     FX_DOUBLE min_step) {
...
FX_VOID FXSMV_OnUpdateV(FX_SmoothValue *obj, FX_DOUBLE target_value,
                        FX_DOUBLE *current_value) {
    FX_DOUBLE tmpv;
    FX_DOUBLE dff;
    FX_DOUBLE old_ratio = 1.0 - obj->m_new_value_ratio;
    tmpv = old_ratio * obj->m_current_value +
           obj->m_new_value_ratio * target_value;
...
FX_VOID FXSMV_OnUpdateVZO(FX_SmoothValue *obj, FX_DOUBLE target_value,
                          FX_DOUBLE *current_value, FX_BOOL *is_zero,
                          FX_BOOL *is_one) {
    FX_DOUBLE tmpv;
    FX_DOUBLE dff;
    FX_DOUBLE old_ratio = 1.0 - obj->m_new_value_ratio;
...
FX_VOID FXSMV_OnUpdateZO(FX_SmoothValue *obj, FX_DOUBLE target_value,
                         FX_BOOL *is_zero, FX_BOOL *is_one) {
    FX_DOUBLE tmpv;
    FX_DOUBLE dff;
    FX_DOUBLE old_ratio = 1.0 - obj->m_new_value_ratio;
    tmpv = old_ratio * obj->m_current_value +
