...
void OnGetServoErr_A(long ErrCode[7]) {
    ...
    if (local_log_tag == true) {
        printf("[Marvin SDK]: A arm Servo error code=[%d,%d,%d,%d,%d,%d,%d],\n",
               ErrCode[0], ErrCode[1], ErrCode[2], ErrCode[3], ErrCode[4],
               ErrCode[5], ErrCode[6]);
    }
}
...
long OnGetSDKVersion() { return CRobot::OnGetSDKVersion(); }
...
long OnSavePara() { return CRobot::OnSavePara(); }
bool OnGetBuf(DCSS *ret) { return CRobot::OnGetBuf(ret); }
...
bool OnStopGather() { return CRobot::OnStopGather(); }
bool OnSaveGatherData(char *path) { return CRobot::OnSaveGatherData(path); }
...
bool OnRelease() { return CRobot::OnRelease(); }

bool OnClearSet() { return CRobot::OnClearSet(); }

bool OnSetTargetState_A(int state) { return CRobot::OnSetTargetState_A(state); }
...
bool OnSetDragSpace_A(int dgType) { return CRobot::OnSetDragSpace_A(dgType); }
bool OnSetForceCtrPara_A(int fcType, double fxDir[6], double fcCtrlPara[7],
                         double fcAdjLmt) {
...
bool OnSetForceCmd_A(double force) { return CRobot::OnSetForceCmd_A(force); }
bool OnSetPVT_A(int id) { return CRobot::OnSetPVT_A(id); }
bool OnSetImpType_A(int type) { return CRobot::OnSetImpType_A(type); }
bool OnSetTargetState_B(int state) { return CRobot::OnSetTargetState_B(state); }
...
bool OnSetDragSpace_B(int dgType) { return CRobot::OnSetDragSpace_B(dgType); }
bool OnSetForceCtrPara_B(int fcType, double fxDir[6], double fcCtrlPara[7],
                         double fcAdjLmt) {
...
bool OnSetForceCmd_B(double force) { return CRobot::OnSetForceCmd_B(force); }
bool OnSetImpType_B(int type) { return CRobot::OnSetImpType_B(type); }
bool OnSetPVT_B(int id) { return CRobot::OnSetPVT_B(id); }

bool OnSetSend() { return CRobot::OnSetSend(); }
...
// bool AscIIToHex(unsigned char ascbuf[256], long len, unsigned char
// hexbuf[256], long& retlen)
//{
//	unsigned char V = 0;
//	retlen = 0;
//	long cnt = 0;
...
// bool OnSetChDataB_hex(unsigned char data_ptr[256], long size_int, long
// set_ch)
//{
//	unsigned char buf2[256];
//     memset(buf2, 0, 256);
...
bool OnClearChDataA() { return CRobot::OnClearChDataA(); }
bool OnClearChDataB() { return CRobot::OnClearChDataB(); }
