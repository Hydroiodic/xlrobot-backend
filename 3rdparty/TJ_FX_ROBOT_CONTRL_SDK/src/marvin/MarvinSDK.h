...
// # ATTENTION
//    1.  请先熟练使用MARVIN_APP
//    或者https://github.com/cynthia-you/TJ_FX_ROBOT_CONTRL_SDK/releases/
//    下各个版本里的FxStation.exe，
//    操作APP可以让您更加了解marvin机器人的操作使用逻辑，便于后期用代码开发。
//    2.  ./demo_linux_win/ 为c++ 和 python
//    的接口使用DEMO。每个demo顶部有该DEMO的案例说明和使用逻辑，请您一定先阅读，根据现场情况修改后运行。
//        这些demo的使用逻辑和使用参数为研发测试使用开发的，仅供参考，并非实际生产代码。
//            比如:
//                a.速度百分比和加速度百分比为了安全我们都设置为百分之十：10，在您经过丰富的测试后可调到全速100。
//                b.参数设置之间sleep 1秒或者500毫秒，
//                实际上参数设置之间小睡1毫秒即可。
...
////////////////////////////////////////////////////////////////////////////////////////////////
// 连接机器人
FX_DLL_EXPORT bool OnLinkTo(FX_UCHAR ip1, FX_UCHAR ip2, FX_UCHAR ip3,
                            FX_UCHAR ip4);
...
// 设置保存参数开始采集数据
FX_DLL_EXPORT bool OnStartGather(long targetNum, long targetID[35],
                                 long recordNum);
// 停止数据采集
FX_DLL_EXPORT bool OnStopGather();

// 设置指定手臂的工具参数:运动学和动力学参数,运动学参数使正解到TCP,
// 动力学使扭矩模式可以正常使用 设置左臂工具的运动学和动力学参数
...
// 2 设置指定手臂的关节阻抗参数, 在扭矩模式关节阻抗模式下,即
// OnSetTargetState_A(3) && OnSetImpType_A(1) 下参数才有意义(以左臂为例)
// 设置左臂工具关节阻抗的刚度和阻尼参数
FX_DLL_EXPORT bool OnSetJointKD_A(double K[7], double D[7]);
// 设置右臂工具关节阻抗的刚度和阻尼参数
FX_DLL_EXPORT bool OnSetJointKD_B(double K[7], double D[7]);
// 3 设置指定手臂的迪卡尔阻抗参数, 在扭矩模式迪卡尔阻抗模式下,即
// OnSetTargetState_A(3) && OnSetImpType_A(2) 下参数才有意义(以左臂为例)
...
// 4 如果使用力控模式,在扭矩模式力控模式下,即 OnSetTargetState_A(3) &&
// OnSetImpType_A(3) 以下两个指令连用 4.1 设置指定手臂的力控参数
// 设置左臂力控参数
FX_DLL_EXPORT bool OnSetForceCtrPara_A(int fcType, double fxDir[6],
                                       double fcCtrlPara[7], double fcAdjLmt);
// 设置右臂力控参数
FX_DLL_EXPORT bool OnSetForceCtrPara_B(int fcType, double fxDir[6],
                                       double fcCtrlPara[7], double fcAdjLmt);
// 4.2 设置指定手臂的力值
// 设置左臂力控目标
...
// 获取左臂末端通信模组回复的数据 ret_ch==1:CANFD  ret_ch==2 COM1  ret_ch==3
// COM2
...
FX_DLL_EXPORT bool OnSetChDataA(unsigned char data_ptr[256], long size_int,
                                long set_ch);
// 获取右臂末端通信模组回复的数据
FX_DLL_EXPORT long OnGetChDataB(unsigned char data_ptr[256], long *ret_ch);
// 发送协议指令给右臂末端模组
FX_DLL_EXPORT bool OnSetChDataB(unsigned char data_ptr[256], long size_int,
                                long set_ch);
