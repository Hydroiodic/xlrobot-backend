...
void CFileOp::SetErr() { m_ErrorTag = true; }
void CFileOp::OnReSetErrorTag() { m_ErrorTag = false; }

bool CFileOp::OnCheckErrorTag() { return m_ErrorTag; }
...
bool CFileOp::OnStateSendSvr(FileIns *ins) {
    ...
    while (wltnum > 0) {
        long wn = fwrite(&ins->m_CellContent[ins->m_CellSize - wltnum], 1,
                         wltnum, m_fp);
        wltnum -= wn;
    }
...
bool CFileOp::OnStateSendCln(FileIns *ins) {
    if (ins->m_InsType != INS_TYPE_Send_Request_Report &&
        ins->m_InsType != INS_TYPE_Send_File_Cell_Report) {
        return false;
    }
    if (ins->m_InsType == INS_TYPE_Send_Request_Report ||
        ...
        if (a_serial < m_SendTotalBlockNum - 1) {
            ...
            while (rltnum > 0) {
                long rn = fread(&ins->m_CellContent[ins->m_CellSize - rltnum],
                                1, rltnum, m_fp);
                rltnum -= rn;
            }
        } else {
...
bool CFileOp::OnStateRecvSvr(FileIns *ins) {
    ...
    if (a_serial < m_SendTotalBlockNum - 1) {
        ...
        long rltnum = ins->m_CellSize;
        while (rltnum > 0) {
            long rn = fread(&ins->m_CellContent[ins->m_CellSize - rltnum], 1,
                            rltnum, m_fp);
            rltnum -= rn;
        }
    } else {
